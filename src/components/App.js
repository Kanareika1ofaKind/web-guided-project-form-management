...
export default function App() {
  ...
  const updateForm = (inputName, inputValue) => {
    ...
    setFormValues({ ...formValues, [inputName]: inputName });
  }
  ...
  const submitForm = () => {
    ...
    if (!newFriend.username || !newFriend.email || !newFriend.role) {
      setFormError("Ya dork, username, email AND role are required. Get outta here!");
    }

    axios.post("fakeapi.com", newFriend)
    .then(res => {
      // console.log(res);   // zdes' chisto tol'ko dlya proverki
